...
function UpdateCampaignMessage() {
  ...
  const updateCampaignMessages = async () => {
    if (isLoading) {
    ...
    try {
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          "empno": empno,
          "message": editorState // Use the campaign message from the state
        })
      });
  ...
  const handlePreview = () => {
    const previewUrl = `/campaignPreview?empno=${empno}&message=${encodeURIComponent(editorState)}`;
          ...
          <div>
            <div className="relative h-[300px] sm:h-[400px] lg:h-[300px] xl:h-[300px] 2xl:h-[300px]">
              ...
              <div className="absolute bottom-[-70px] left-1/2 transform translate-x-[-50%]">
                <div className='border p-5 bg-red-500'>
                    <h1 className='font-bold text-white'>Note: You cannot update your profile picture</h1>
                </div>
    ...
    >
      Change Picture
        ...
        <div className="flex justify-center items-center space-x-5">
          ...
          {/* {showSubmitButton && ( */}
                    <Button onClick={updateCampaignMessages} className="mt-5 mb-10 bg-[#1E2C8A]">
                      Publish
                    </Button>
                  {/* )} */}
          </div>
          <div>
            <Button onClick={handlePreview} className="mt-5 mb-10 bg-[#1E2C8A]">
                Preview Page
            </Button>
          </div>
...
export default UpdateCampaignMessage
